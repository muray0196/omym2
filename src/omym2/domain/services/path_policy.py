...
class PathPolicy:
    ...
    def _album(self, metadata: TrackMetadata) -> str:
        ...
        if not self.config.sanitize:
            return value
        return sanitize_album_name(value)
    ...
    def _artist_component(self, value: str) -> str:
        if not self.config.sanitize:
            return value
        return sanitize_artist_name(value)
...
def sanitize_path_component(
    ...
    *,
    ...
    return sanitize_string(value, max_length=max_length, preserve_extension=preserve_extension)
...
def _limit_sanitized_with_extension(base: str, extension_suffix: str, max_length: int | None) -> str:
    if max_length is None:
        return f"{base}{extension_suffix}"
    if max_length <= 0:
        return ""

    extension_bytes = _utf8_length(extension_suffix)
    if extension_bytes > max_length:
        return ""

    limited_base = _limit_utf8(base, max_length - extension_bytes).strip(SANITIZER_REPLACEMENT)
