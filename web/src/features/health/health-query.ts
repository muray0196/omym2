...
export function checkIssuesInfiniteQuery(filters: HealthFilters) {
  return infiniteQueryOptions({
    ...
    queryFn: ({ pageParam, signal }): Promise<CheckIssuesData> =>
      readIssues(filters, pageParam, signal),
    queryKey: [
      "check",
      filters.query,
...
export function checkIssueFacetsQuery(filters: HealthFilters) {
  return queryOptions({
    queryFn: ({ signal }) => readFacets(filters, signal),
...
export function checkIssueGroupsInfiniteQuery(filters: HealthFilters) {
  return infiniteQueryOptions({
    ...
    getNextPageParam: (lastPage: CheckIssueGroupsData) =>
      lastPage.page.next_cursor ?? undefined,
    queryFn: ({ pageParam, signal }): Promise<CheckIssueGroupsData> =>
      readGroups(filters, pageParam, signal),
    queryKey: [
      "check",
      "groups",
      filters.groupBy,
