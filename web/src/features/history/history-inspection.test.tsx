...
describe("Run detail inspection", () => {
  it("shows pending mutation evidence and a disabled Undo control with recovery", async () => {
    ...
    expect(
      await screen.findByText(/review this event manually/i),
    ).toBeVisible();
    expect(screen.getByRole("heading", { name: "Run detail" })).toBe(
      loadingHeading,
    );
  ...
  it("clears an active FileEvent status through the All option", async () => {
    const requestedStatuses: Array<string | null> = [];
    server.use(
      http.get("*/api/history/:runId", () => HttpResponse.json(runDetail)),
      http.get("*/api/history/:runId/events", ({ request }) => {
        requestedStatuses.push(new URL(request.url).searchParams.get("status"));
        return HttpResponse.json(runEvents);
      }),
      http.get("*/api/history/:runId/events/facets", () =>
        HttpResponse.json(eventFacets),
      ),
    ...
    const { router, user } = renderRoute(
      `/history/${RUN_ID}?event_status=failed`,
    );
    ...
    await waitFor(() => expect(requestedStatuses).toContain("failed"));
    await user.selectOptions(status, "");

    await waitFor(() => expect(router.state.location.search).toBe(""));
    await waitFor(() => expect(requestedStatuses.at(-1)).toBeNull());
    expect(status).toHaveValue("");
  });

  it("round-trips History list URL state through detail and Back", async () => {
    server.use(
      http.get("*/api/history", () => HttpResponse.json(historyList)),
      http.get("*/api/history/facets", () => HttpResponse.json(historyFacets)),
      http.get("*/api/history/:runId", () => HttpResponse.json(runDetail)),
      http.get("*/api/history/:runId/events", () =>
        HttpResponse.json(runEvents),
      ),
    ...
    const initialSearch = "?query=partial&status=partial_failed";
    const { router, user } = renderRoute(`/history${initialSearch}`);

    await user.click(
      await screen.findByRole("link", { name: new RegExp(RUN_ID) }),
    );

    expect(router.state.location.pathname).toBe(`/history/${RUN_ID}`);
    expect(router.state.location.search).toBe(initialSearch);
    const back = await screen.findByRole("link", { name: "Back to History" });
    expect(back).toHaveAttribute("href", `/history${initialSearch}`);
...
const historyList = {
  data: {
    items: [runDetail.data.run],
    page: { limit: 100, next_cursor: null, total: 1 },
  },
  errors: [],
} satisfies ApiEnvelopePaginatedDataRunHeader;

const historyFacets = {
  data: {
    facets: { status: [{ count: 1, value: "partial_failed" }] },
    total: 1,
  },
  errors: [],
