...
export function plansInfiniteQuery(filters: PlanListFilters) {
  ...
  >({
    ...
    queryFn: ({ pageParam, signal }) =>
      readPlansPage(filters, pageParam, signal),
    queryKey: [
      "plans",
      filters.query,
      filters.status ?? null,
...
export function exactPlanQuery(planId: string) {
  return queryOptions({
    ...
    queryFn: ({ signal }) => readPlanDetail(planId, signal),
...
export function planActionsInfiniteQuery(
  ...
  >({
    ...
    getNextPageParam: (lastPage) => lastPage.page.next_cursor ?? undefined,
    queryFn: ({ pageParam, signal }) =>
      readPlanActionsPage(planId, filters, pageParam, signal),
    queryKey: [
      "plans",
      planId,
      "actions",
...
export function planActionFacetsQuery(
  ...
  return queryOptions({
    enabled: planId.length > 0,
    queryFn: ({ signal }) => readPlanActionFacets(planId, filters, signal),
    queryKey: [
      "plans",
      planId,
      "facets",
...
export function planActionGroupsInfiniteQuery(
  ...
  >({
    ...
    getNextPageParam: (lastPage) => lastPage.page.next_cursor ?? undefined,
    queryFn: ({ pageParam, signal }) =>
      readPlanActionGroupsPage(planId, filters, pageParam, signal),
    queryKey: [
      "plans",
      planId,
      "groups",
