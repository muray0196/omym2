...
def test_operation_baseline_has_exact_columns_and_indexes(tmp_path: Path) -> None:
    """The clean baseline creates only the active Operation persistence shape."""
...
def test_sqlite_operation_repository_round_trips_errors_and_candidates(tmp_path: Path) -> None:
    """Interrupted evidence is retained but not repeatedly reconciled."""
    database_file = default_application_paths(tmp_path).database_file
...
def test_sqlite_operation_repository_reconciles_interrupted_apply_only_with_unfinished_managed_state(
    ...
    *,
    ...
    database_file = default_application_paths(tmp_path).database_file
    plan = replace(_plan(), status=PlanStatus.APPLYING) if plan_applying else _plan().mark_failed()
