...
def test_file_mover_falls_back_for_cross_device_moves(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """FileMover supports Incoming and Library paths on different filesystems."""
    source_path = tmp_path / AUDIO_FILE_NAME
    target_path = tmp_path / "Artist" / TARGET_FILE_NAME
    _ = source_path.write_bytes(AUDIO_CONTENT)
    original_rename = type(source_path).rename

    def raise_cross_device_once(self: Path, target: Path) -> Path:
        if self == source_path and target == target_path:
            raise OSError(EXDEV, "Invalid cross-device link")
        return original_rename(self, target)

    monkeypatch.setattr(type(source_path), "rename", raise_cross_device_once)
