...
from omym2.domain.models.app_config import ArtistIdConfig, ArtistIdEntry, PathPolicyConfig
...
EXPECTED_ARTIST_ID_PATH = "AMR/Example-Song.flac"
...
def test_path_policy_renders_artist_id_placeholder_from_saved_config() -> None:
    """PathPolicy can use editable saved artist IDs without external lookup."""
    artist_ids = ArtistIdConfig(entries=(ArtistIdEntry(source_artist=ALBUM_ARTIST, artist_id="AMR"),))

    canonical_path = PathPolicy(PathPolicyConfig(template=ARTIST_ID_TEMPLATE), artist_ids).canonical_path(
        _track_metadata(),
        FILE_EXTENSION,
    )
