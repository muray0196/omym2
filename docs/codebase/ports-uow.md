...
* `ArtistNameLookup` (owned by the `artist_names` feature) — `search_artists(source_name)` returns scored provider candidates with MusicBrainz identities, canonical names, and alias facts. Ports report typed external facts; ambiguity, display-name tiering, stickiness, and fallback stay in the feature resolver. Distinct from the `ArtistNameProvider` `StrEnum` (`musicbrainz` | `user`) that records DB row provenance in [DB Schema](../contracts/db-schema.md#accepted_artist_names).
