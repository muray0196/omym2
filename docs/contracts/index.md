...
* [DB Schema Contract](db-schema.md) - Defines OMYM2's SQLite tables, accepted artist-name provenance, PlanAction naming diagnostics, durable Operation schema, atomic Apply reservation, undo provenance, forward-only migrations, indexes, JSON boundaries, and timestamp policy.
...
* [Web API Contract](web-api.md) - Defines the bundled local Web API's typed envelopes, errors, artist-name settings and Plan diagnostics, settings concurrency, bootstrap, durable-operation routes, capabilities, and preserved browsing semantics.
