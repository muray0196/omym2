...
* `GET /api/plans?query=&status=&type=&blocked=&limit=&cursor=` — list envelope of `PlanSummary` items. `query` matches `plan_id`, `library_id`, `plan_type`, or `status` as a case-insensitive substring. `blocked=true` selects Plans whose persisted summary records at least one blocked action; omitted or `false` leaves blocked count unfiltered. Search combines with status, type, and blocked-only selection before pagination.
...
* `GET /api/history?query=&status=&plan_id=&library_id=&limit=&cursor=` — list envelope of `RunSummary` items. `query` matches `run_id`, `plan_id`, `library_id`, `status`, or non-null `error_summary` as a case-insensitive substring. `plan_id` remains a separate exact Plan identity filter, and all filters combine before pagination.
