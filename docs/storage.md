...
The DB stores Library registration state. Registration is distinct from Track rows and is not defined by whether the `tracks` table has rows.
...
* persisting Library registration state
...
The current acceptance state of the configured Library under the current resolved Library root and current PathPolicy.
...
`path_policy_hash` may be replaced by an equivalent config identity that represents the current PathPolicy. Changing PathPolicy invalidates prior registration.

Library registration is distinct from `tracks`. Track rows record OMYM2's last known managed files; they do not prove that the current Library root and current PathPolicy have been accepted.

DB-only registration updates are not Library music file mutations and do not create FileEvents.
