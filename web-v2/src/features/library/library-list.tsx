...
export function LibraryList() {
  ...
  const tracks = useInfiniteQuery(
    tracksInfiniteQuery(libraryId, browse.filters),
  );
  const facets = useQuery(trackFacetsQuery(libraryId, browse.filters));
  const groups = useInfiniteQuery(
    trackGroupsInfiniteQuery(libraryId, browse.filters),
  );

  const trackItems = tracks.data?.pages.flatMap((page) => page.items) ?? [];
    ...
    <section aria-labelledby="track-groups" className={styles.section}>
      ...
      {hasNextPage ? (
        <Button disabled={isFetchingNextPage} onClick={onLoadMore}>
          {isFetchingNextPage
            ? libraryCopy.list.loadingMoreGroups
            : libraryCopy.list.loadMoreGroups}
        </Button>
      ) : null}
    </section>
  );
  ...
  return (
    <section aria-labelledby="track-results" className={styles.section}>
      ...
      {hasNextPage ? (
        <Button disabled={isFetchingNextPage} onClick={onLoadMore}>
          {isFetchingNextPage
            ? libraryCopy.list.loadingMore
            : libraryCopy.list.loadMore}
        </Button>
      ) : null}
    </section>
  );
