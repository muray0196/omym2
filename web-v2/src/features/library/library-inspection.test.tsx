...
describe("Library inspection", () => {
  ...
  it("passes the backend cursor back unchanged when loading another Track page", async () => {
  ...
  it("renders persisted metadata, paths, hashes, and a browse-state-preserving back link", async () => {
    ...
    expect(
      await screen.findByRole("heading", { name: "So What" }),
    ).toBeVisible();
    ...
    ).toHaveAttribute("href", "/library?query=So+What&status=active");
    expect(
      screen.queryByRole("button", { name: /refresh/i }),
    ).not.toBeInTheDocument();
  });
...
function groupedTracks(
  ...
  return {
    data: {
      group_by: groupBy,
      items,
      page: { limit: 100, next_cursor: null, total: items.length },
    },
    errors: [],
