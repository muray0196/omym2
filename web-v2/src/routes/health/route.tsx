...
export function Component() {
  const { clearGroup, filters, hasActiveFilters, resetFilters, updateFilters } =
    useHealthFilters();
  const issuesQuery = useInfiniteQuery(checkIssuesInfiniteQuery(filters));
  const facetsQuery = useQuery(checkIssueFacetsQuery(filters));
  const groupsQuery = useInfiniteQuery(checkIssueGroupsInfiniteQuery(filters));
        ...
        <div className={styles.actions}>
          {filters.groupKey ? (
            <p className={styles.selected}>
              {healthCopy.selectedGroup}: {filters.groupKey}
            </p>
          ) : null}
          {filters.groupKey ? (
          ...
          <ul className={styles.facetList}>
            {facetsQuery.data.facets.issue_type.map((facet) => (
              <li className={styles.facet} key={facet.value}>
                <span>{issueTypeLabel(facet.value)}</span>
